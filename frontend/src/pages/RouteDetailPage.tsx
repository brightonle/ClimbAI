  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <Link to="/routes" className="text-gray-500 hover:text-gray-700 text-sm">← Routes</Link>
      </div>
          ...
          <div className="bg-white border border-gray-200 shadow-sm rounded-xl p-5">
            ...
            <div className="flex flex-wrap gap-3 mt-2">
              ...
              {route.wall_angle != null && (
                <span className="bg-gray-100 border border-gray-300 text-gray-600 px-3 py-1 rounded-full text-sm">
                  {route.wall_angle}°
                </span>
              )}
            </div>
            {route.description && (
              <p className="text-gray-600 text-sm mt-3">{route.description}</p>
            )}
          </div>

          {/* Attempt stats */}
          {routeAttempts.length > 0 && (
            <div className="bg-white border border-gray-200 shadow-sm rounded-xl p-5 grid grid-cols-3 gap-4 text-center">
              <div>
                <div className="text-2xl font-bold text-gray-900">{routeAttempts.length}</div>
                <div className="text-xs text-gray-500 mt-0.5">Attempts</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-brand-600">{sends}</div>
                <div className="text-xs text-gray-500 mt-0.5">Sends</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-gray-900">{sendRate}%</div>
          ...
          <div className="bg-white border border-gray-200 shadow-sm rounded-xl p-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {route.route_holds.map((rh) => (
                <div key={rh.id} className="bg-gray-100 border border-gray-200 rounded-lg px-3 py-2 flex items-center gap-2 text-sm">
                  <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0 ${
                    rh.position_in_route === 1 ? 'bg-brand-600' :
                    rh.position_in_route === route.route_holds.length ? 'bg-amber-500' : 'bg-blue-600'
                  }`}>
                    {rh.position_in_route}
                  </span>
                  <div className="min-w-0">
                    <div className="text-gray-700 truncate">{rh.hold.hold_type || 'Hold'}</div>
                    {rh.hold.function && (
              ...
              <div className="space-y-2">
                {routeAttempts.slice(0, 20).map((a) => (
                  <div key={a.id} className="flex items-center gap-3 text-sm">
                    ...
                    <span className={a.success ? 'text-brand-600' : 'text-red-500'}>
                      {a.success ? 'Send' : 'Fell'}
                    </span>
            ...
            <div className="bg-white border border-gray-200 shadow-sm rounded-xl p-4">
              ...
              <div className="space-y-2">
                {similar.map((r: Route) => (
                  ...
                  >
                    <span className="text-gray-900 text-sm font-medium flex-1 truncate">{r.name}</span>
                    {r.difficulty_grade && (
                      <span
                        className="text-xs px-2 py-0.5 rounded font-mono flex-shrink-0"
                        style={gradeBadgeStyle(r.difficulty_grade)}
                      >
                        {r.difficulty_grade}
                      </span>
                    )}
